...
- **Resolved commands** (rendered per-repo from the distribution registry):
  ...
  - Validate/verify/build command: `npm run build`
...
### 5. Validate
- Run the repo's validator/build: `npm run build` (or the
...
- `npm run build` (or resolved validator) and the full suite pass ✓
